...
for i = 1:length(files) %iterate through experiment folder
    ...
    if ismember(rat,Rat) && any(strcmp(session, pun_early)) 
...
%___________ Dimensions and labels used for the plots ______________
    ...
    time = linspace(-10, 45, size(Rew_Resp_E, 2));
    Rew_E_completed = num2str(size(Rew_Resp_E,1));
    Rew_E_Resp_label = ['Early Reward completions (', Rew_E_completed,')'];
    Rew_L_completed = num2str(size(Rew_Resp_L,1));
    Rew_L_Resp_label = ['Late Reward completions (', Rew_L_completed,')'];

    Pun_E_completed = num2str(size(Pun_Resp_E,1));
    Pun_E_Resp_label = ['Early Punish completions (', Pun_E_completed,')'];
    Pun_L_completed = num2str(size(Pun_Resp_L,1));
    Pun_L_Resp_label = ['Late Punish completions (', Pun_L_completed,')'];
...
[perm.rewE_punE, ~] = permTest_array(Rew_Resp_E, Pun_Resp_E, 1000);
[perm.rewL_punL, ~] = permTest_array(Rew_Resp_L, Pun_Resp_L, 1000);
...
labels = {Rew_E_Resp_label, Rew_L_Resp_label};
colors = {blue, azure};
...
labels = {Pun_E_Resp_label, Pun_L_Resp_label};
colors = {red, pink};
