...
%Boxes 1 and 2
% Rat = {'R09','R10','R13','R14','R17','R22'};
%Boxes 3 and 4
Rat = {'R11','R12','R15','R16','R19','R20','R23', 'R24'};
...
for i = 1:length(files) %iterate through experiment folder
    ...
    if any(strcmp(session, conf_early)) && ismember(rat,Rat) 
        for j = 1:size(sesdat.traces_z, 1)
             if isnan(sesdat.traces_z(j,5))
             elseif sesdat.traces_z(j, 2) == 5 && sesdat.traces_z(j, 4) == 1 
                Rew_Resp_E = [Rew_Resp_E; sesdat.traces_z(j, 5:end)];
             elseif sesdat.traces_z(j, 2) == 6 && sesdat.traces_z(j, 4) == 2 
                Pun_Resp_E = [Pun_Resp_E; sesdat.traces_z(j, 5:end)];
             elseif sesdat.traces_z(j, 2) == 5 && sesdat.traces_z(j, 4) == 3 
                Conf_Resp_E = [Conf_Resp_E; sesdat.traces_z(j, 5:end)];
                 Conf_Resp_F_E = [Conf_Resp_F_E; sesdat.traces_z(j, 5:end)];
             elseif sesdat.traces_z(j, 2) == 6 && sesdat.traces_z(j, 4) == 3 
                Conf_Resp_E = [Conf_Resp_E; sesdat.traces_z(j, 5:end)];
                 Conf_Resp_S_E = [Conf_Resp_S_E; sesdat.traces_z(j, 5:end)];
             end
        end
    end

    if any(strcmp(session, conf_late)) && ismember(rat,Rat) 
        for j = 1:size(sesdat.traces_z, 1)
             if isnan(sesdat.traces_z(j,5))
             elseif sesdat.traces_z(j, 2) == 5 && sesdat.traces_z(j, 4) == 1 
                Rew_Resp_L = [Rew_Resp_L; sesdat.traces_z(j, 5:end)];
             elseif sesdat.traces_z(j, 2) == 6 && sesdat.traces_z(j, 4) == 2 
                Pun_Resp_L = [Pun_Resp_L; sesdat.traces_z(j, 5:end)];
             elseif sesdat.traces_z(j, 2) == 5 && sesdat.traces_z(j, 4) == 3 
                Conf_Resp_L = [Conf_Resp_L; sesdat.traces_z(j, 5:end)];
                 Conf_Resp_F_L = [Conf_Resp_F_L; sesdat.traces_z(j, 5:end)];
             elseif sesdat.traces_z(j, 2) == 6 && sesdat.traces_z(j, 4) == 3 
                Conf_Resp_L = [Conf_Resp_L; sesdat.traces_z(j, 5:end)];
                 Conf_Resp_S_L = [Conf_Resp_S_L; sesdat.traces_z(j, 5:end)];
             end
        end
    end
end
...
%___________ Dimensions and labels used for the plots ______________
    ...
    Rew_Resp_E_label = ['Early Reward (', Rew_Resp_E_completed,')'];
    Pun_Resp_E_completed = num2str(size(Pun_Resp_E,1));
    Pun_Resp_E_label = ['Early Punish (', Pun_Resp_E_completed,')'];
    Conf_Resp_E_completed = num2str(size(Conf_Resp_E,1));
    Conf_Resp_E_label = ['Early Conf (', Conf_Resp_E_completed,')'];
    Conf_Resp_E_completed_F = num2str(size(Conf_Resp_F_E,1));
    Conf_Resp_E_F_label = ['Early Conf + food (', Conf_Resp_E_completed_F,')'];
    Conf_Resp_E_completed_S = num2str(size(Conf_Resp_S_E,1));
    Conf_Resp_E_S_label = ['Early Conf + shock (', Conf_Resp_E_completed_S,')'];

    Rew_Resp_L_completed = num2str(size(Rew_Resp_L,1));
    Rew_Resp_L_label = ['Late Reward (', Rew_Resp_L_completed,')'];
    Pun_Resp_L_completed = num2str(size(Pun_Resp_L,1));
    Pun_Resp_L_label = ['Late Punish (', Pun_Resp_L_completed,')'];
    Conf_Resp_L_completed = num2str(size(Conf_Resp_L,1));
    Conf_Resp_L_label = ['Late Conf (', Conf_Resp_L_completed,')'];
    Conf_Resp_L_completed_F = num2str(size(Conf_Resp_F_L,1));
    Conf_Resp_L_F_label = ['Late Conf + food (', Conf_Resp_L_completed_F,')'];
    Conf_Resp_L_completed_S = num2str(size(Conf_Resp_S_L,1));
    Conf_Resp_L_S_label = ['Late Conf + shock (', Conf_Resp_L_completed_S,')'];
...
title('Conflict Early v Late');  


%-----------------------------------------------------Plot Reward Early v Late -----------------------------------------------------------------------
...
line([ax.XLim(1), ax.XLim(2)], [0, 0], 'Color', 'black', 'HandleVisibility', 'off');
line([0, 0], [yLimits(1), yLimits(2)], 'Color', 'black', 'HandleVisibility', 'off');
title('Reward Early v Late');  

%-----------------------------------------------------Plot Punish Early v Late -----------------------------------------------------------------------
...
line([ax.XLim(1), ax.XLim(2)], [0, 0], 'Color', 'black', 'HandleVisibility', 'off');
line([0, 0], [yLimits(1), yLimits(2)], 'Color', 'black', 'HandleVisibility', 'off');
title('Punish Early v Late');  

% %-----------------------------------------------------Conflict Food Early v Late -----------------------------------------------------------------------
...
line([ax.XLim(1), ax.XLim(2)], [0, 0], 'Color', 'black', 'HandleVisibility', 'off');
line([0, 0], [yLimits(1), yLimits(2)], 'Color', 'black', 'HandleVisibility', 'off');
title('Conflict Food Early v Late');  
...
line([ax.XLim(1), ax.XLim(2)], [0, 0], 'Color', 'black', 'HandleVisibility', 'off');
line([0, 0], [yLimits(1), yLimits(2)], 'Color', 'black', 'HandleVisibility', 'off');
title('Conflict Shock Early v Late');  
